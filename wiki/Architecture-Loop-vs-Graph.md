...
A graph covers independent workstreams, separate rubrics for each stage, state that must survive beyond a single session, or a high-consequence write. Its nodes are typed as either `agent` or `human`; see [GRAPH.md](https://github.com/tqnonline/skills/blob/main/skills/developer/conduct/GRAPH.md) for the two schemas. Its edges are explicit handoffs of a defined artifact — never an assumption that the previous step has already finished.
