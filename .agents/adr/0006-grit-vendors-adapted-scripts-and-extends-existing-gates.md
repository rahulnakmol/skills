...
**3. The depth tree governs verification decomposition only.** `grit`'s depth tree — 5 to 10 layers, used to decompose verification for substantial work — decides how finely a piece of work's acceptance criteria are broken into checks. It has no say over execution shape. `orchestrate` keeps sole authority over whether work runs as a loop, a graph, or a hybrid of the two. A gate is a ledger row, not an agent: deepening a ledger adds checks to verify against, not agents to run the work. A ten-layer depth tree therefore does not conflict with the orchestrate rubric's "count tokens, not agents" — it can add cost only in checks executed, never in agents spawned on its own authority.

## Consequences

- `NOTICE` carries the attribution for `skills/developer/grit/scripts/` and `adapters/claude/hooks/`, naming unlazy, its copyright holder, and its MIT license.
