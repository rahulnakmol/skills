...
test('llms.txt lists exactly the 34 manifest skills', () => {
  const plugin = JSON.parse(read('.claude-plugin/plugin.json'));
  const manifestNames = plugin.skills.map((p) => p.split('/').pop());
  assert.equal(manifestNames.length, 34, `expected 34 manifest skills, found ${manifestNames.length}`);
  ...
  assert.equal(listedSet.size, 34, `site/llms.txt must list exactly 34 skills, found ${listedSet.size}`);
});
