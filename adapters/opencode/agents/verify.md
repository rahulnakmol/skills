...
Check outcome/value trace, allowed scope, changed paths, contracts, invariants, failure paths, tests added, architecture/ADR constraints, security/data/privacy, operability, accessibility/performance where relevant, migration/rollback, and parent/CI verification plan. Refute unsupported claims and cite exact evidence. Where a grit gate ledger exists for the subject (for example `skills/developer/grit/LEDGER.md`), run its gates with `gate-check.mjs --status` and include the met, unmet, and abandoned counts in the verdict; unmet gates preclude `HANDOFF_READY`.
