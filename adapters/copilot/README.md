...
Copilot has no hook runtime to intercept a session at the point it would report a task complete, so enforcement here relies on two mechanisms instead of one: an instruction in `.github/copilot-instructions.md`, and the continuous-integration backstop at `skills/developer/grit/grit-gates.yml`.
