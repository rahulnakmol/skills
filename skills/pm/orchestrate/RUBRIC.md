...
Every routing decision names which shape it chose and why, in the same one-line form the developer rubric uses: mode, and the routing question that decided it. Model selection per step still goes through `model-routing` — a grill round's question-generation step and a research agent's fact-gathering step do not need the same tier.
