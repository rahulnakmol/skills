...
import {
  GRIT_DIR, appendStatus, claimLeases, formatDocument, gateState,
...
run modes:
  ...
  --shell PATH          command shell (GRIT_SHELL, then platform default)
...
pipeline actions:
  ...
  --list-scopes                      list .grit pipelines

targeting:
  --scope ID             use .grit/ID (or GRIT_SCOPE)
...
Approvals live outside the repository under ~/.grit/approved by default.
...
if (action === "--list-scopes") {
  ...
  } else console.log("(no pipelines under " + GRIT_DIR + "/)");
  process.exit(0);
}
...
if (!target.files.length && action !== "--release") {
  failUsage("no gate files found (looked for " + GRIT_DIR + "/<scope>/, then GATES.md and gates/*.md under " + root + ")");
}
...
function resolveShell(raw) {
  const requested = raw || process.env.GRIT_SHELL || (process.platform === "win32" ? (process.env.ComSpec || "cmd.exe") : "/bin/sh");
...
const approvalDir = resolve(process.env.GRIT_APPROVAL_DIR || join(homedir(), ".grit", "approved"));
const canonicalRoot = realpathSync(root);
if (!opt.status && pathIsInside(root, approvalDir)) failUsage("GRIT_APPROVAL_DIR must be outside the repository root");
