...
export const GRIT_DIR = ".grit";
export const LOCK_DIR = join(GRIT_DIR, "locks");
...
export function scopeRoot(root, scope) {
  return join(root, GRIT_DIR, scope);
}

export function listScopes(root) {
  const directory = join(root, GRIT_DIR);
...
export function resolveTarget(options = {}) {
  ...
  const wanted = options.scope || process.env.GRIT_SCOPE || null;
  if (wanted) {
    ...
    if (!scopes.includes(wanted)) {
      ...
      const statePath = join(root, GRIT_DIR);
      ...
      return {
        ...
        error: "no such scope \"" + wanted + "\" under " + GRIT_DIR + "/ (have: " +
          (scopes.join(", ") || "none") + ")",
      };
    }
    return targetFromDiscovery("scope", wanted, scopeDiscovery(root, wanted));
  ...
  if (scopes.length > 1) {
    ...
    return {
      ...
      error: scopes.length + " pipelines present (" + scopes.join(", ") +
        "); pass --scope <id> or set GRIT_SCOPE. Refusing to guess.",
    };
  }
...
export function statusLogPath(root, scope) {
  return scope ? join(scopeRoot(root, scope), "status.log") : join(root, "grit-status.log");
}

export function hookStatePath(root, scope) {
  return scope ? join(scopeRoot(root, scope), "hook-state.json") : join(root, ".grit-hook-state.json");
}

function assertSafeStatePath(root, target) {
  const stateRoot = join(resolve(root), GRIT_DIR);
...
function lockDirectory(root) {
  ...
  // namespace. The root itself must exist for any grit operation.
