...
<div class="tool-block-body">
<p>Copilot's agent mode reads the same <code>.agents/skills/</code> catalog, driven by <code>.github/copilot-instructions.md</code>. Copilot has no hook runtime to intercept a session at completion, so enforcement relies on a standing instruction in that file, plus the <code>grit-gates.yml</code> continuous-integration check, which lints the ledger and fails independently of what the agent reports.</p>
...
Yes, with different enforcement in each tool. Claude Code's opt-in stop hook can block a session directly. OpenCode's `grit-verify` command and its verify agent's rule refuse a handoff-ready verdict while a gate is unmet. Cursor, Codex, and GitHub Copilot have no hook runtime at all, so each relies on an instruction file an agent could in principle skip. The backstop is the same `grit-gates.yml` continuous-integration check, which lints the ledger and fails independently of what the agent reports — once a maintainer has wired it to pull requests, since it ships dispatch-only.
