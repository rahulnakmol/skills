...
<div class="prompt-card">specs/product-constitution.md doesn't exist yet, and QuenServe's E1 initiative is about to go through Investment without one. Start Create mode the way skills/pm/constitution/CONSTITUTION.md sequences it — the seven sections worked in small batches, each principle argued as a real trade-off rather than a platitude — then assemble the Tier 1 summary once every section file is written.<button type="button" class="prompt-card-copy" aria-label="Copy this prompt">Copy</button></div>
