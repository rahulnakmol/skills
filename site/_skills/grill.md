...
<div class="prompt-card">QuenServe epic E1's case needs grilling before the sponsor sees it. Run it the way skills/pm/grill/GRILL-PM.md's round protocol works — themed rounds of three to five questions, scope restated after each one — then stop for the pre-gate blind-spot checklist before recommending any sign-off.<button type="button" class="prompt-card-copy" aria-label="Copy this prompt">Copy</button></div>
