...
<div class="prompt-card">All I've got on QuenServe epic E1 is a raw problem statement from the operations team — nothing scoped, no owner, no PRD yet. Match it against the routing map in skills/pm/ask-pm/SKILL.md and tell me which pm skill to run first, and why that one beats the alternatives.<button type="button" class="prompt-card-copy" aria-label="Copy this prompt">Copy</button></div>
