...
<div class="prompt-card">QuenServe epic E1's PRD already cleared prd-draft's structural check and is headed for the Quality gate next. Score it the way skills/pm/prd-review/ELEVEN-STAR.md's framework runs — every feature mapped to a star level before anything else gets scored — then hand back the seven-dimension composite and name the verdict band it lands in.<button type="button" class="prompt-card-copy" aria-label="Copy this prompt">Copy</button></div>
