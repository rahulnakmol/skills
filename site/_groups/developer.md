...
The developer group covers the delivery pipeline end to end. That starts with orientation on an existing estate or intake of a new idea, a signed PRD, and a decomposed and published backlog. It continues through a gated build loop with human approval at every consequential step, technical design, security hardening, release readiness, post-release reliability, and a sandboxed review before merge. Fifteen skills carry that pipeline. Four of them — `recon`, `impact`, `sdlc`, and `shakedown` — make up the "deliver with evidence" journey, which runs from first orientation to a reviewed pull request and takes in `grit` from the shared core group at its fourth step. The rest handle routing, model selection, backlog mechanics, design, security, and operations around them.
